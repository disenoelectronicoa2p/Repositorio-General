...
float temp,hum,currentHum; 
long currentTemp;

int err;
String data, dato;
String protocol, uri, auth;
...
void loop() {
  ...
  while (!validateConfig) {
    ...
    if (answer) {
      ...
      if(answer){
        answer = enviarComandoAT("AT+HTTPSSL=1", "OK", 2000); //Se configura que es una solicitud HTTPS
        if (answer) {
          answer = enviarComandoAT("AT+HTTPPARA=\"CID\",1", "OK", 4000); //Configura el Context Identifier (CID) para la conexión HTTP
          if (answer) {
            Serial2.println("AT+HTTPPARA=\"URL\",\"" + protocol + "://" + uri + "?auth=" + auth + "\"");
            Serial.println("AT+HTTPPARA=\"URL\",\"" + protocol + "://" + uri + "?auth=" + auth + "\"");
  ...
  if ((unsigned long)(tiempoActualMSN - tiempoPrevioMSN) >= measurementTime) {
    ...
    Serial2.println("AT+HTTPACTION=1"); //Se realizará una solicitud HTTPS POST
    Serial.println("AT+HTTPACTION=1");
    enviarComandoAT("", "", 8000);
   
    bool answer = false;
    answer = enviarComandoAT("AT+HTTPREAD", "name", 4000);
    if(answer){
...
void sendAtConectionRed() {
  ...
  while ( !answerAtConectionRed ) {
    ...
    if (errorAtConectionRed > 8) {
      ...
      enviarComandoAT("AT+CFUN=1,1", "OK", 6000); //Activa el módulo SIM800L en su modo completo y lo reinicia
      delay(4000);
    }
